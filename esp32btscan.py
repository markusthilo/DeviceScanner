...
class SerialReceiver:
	...
	def readblock(self, marker_begin, marker_end):
...
class ESP32BTScan(SerialReceiver):
	'Use ESP32 device to scan for Bluetooth'

	def __init__(self, port='/dev/ttyUSB0', baudrate=115200, timeout=10, manufacturer=Manufacturer(None)):
		super().__init__(port=port, baudrate=baudrate, timeout=timeout)

	def read(self):
		'Read detected devices from serail port'
		for device in self.readblock(b'>>>devices\r\n', b'>>>done\r\n'):
			print(device)
...
if __name__ == '__main__':	# start here if called as application
	argparser = ArgumentParser(description='Bluetooth Scsnner')
	argparser.add_argument('-c', '--classic', action='store_true',
		help='Scan for classic Bluetooth devices only'
	)
	argparser.add_argument('-e', '--esp32', action='store_true',
		help='Scan using ESP32 device on serial port \n --- UNDER CONSTRUCTION ---'
	)
	...
	if args.esp32:	# use esp32 device?
		btscan = ESP32BTScan(timeout=args.scantime, manufacturer=manuf)
		btscan.read()
		SysExit(0)
